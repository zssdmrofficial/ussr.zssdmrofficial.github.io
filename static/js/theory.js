...
(function (g, f) {
  ...
  `
                            ...
                            (r[r.length - 1] = y),
                                (s =
                                    s.substring(0, s.length - T.raw.length) + y.raw),
  ...
  `
                            ...
                            (r[r.length - 1] = y),
                                (s =
                                    s.substring(0, s.length - g.raw.length) + y.raw),
                                (i =
                                    i.substring(0, i.length - T.raw.length) + y.raw),
  ...
  `),
                        ...
                        i.loose ||
                            (o ? (i.loose = !0) : this.rules.other.doubleBlankLine.test(p) && (o = !0));
                        let y = null,
                            ee;
                        this.options.gfm &&
                    ...
                    for (let c = 0; c < i.items.length; c++)
                        if (
                            ((this.lexer.state.top = !1),
                                (i.items[c].tokens = this.lexer.blockTokens(
                                    i.items[c].text,
                                    []
                                )),
                                !i.loose)
  ...
  `
            ...
            emStrong(e, t, n = "") {
                ...
                if (
                    !s ||
                    (s[3] && n.match(this.rules.other.unicodeAlphaNumeric))
                )
                    return;
                if (
                    !(s[1] || s[2] || "") ||
...
async function loadMarkdownContent(markdown, containerId) {
    try {
        const container = document.getElementById(containerId);
        container.innerHTML = marked.parse(markdown);

    } catch (err) {
        console.error("渲染 Markdown 時發生錯誤：", err);
        const container = document.getElementById(containerId);
        if (container) {
            container.innerHTML =
...
window.addEventListener("DOMContentLoaded", () => {
    loadMarkdownContent(inlineMarkdown, "theory-text-container");
});
